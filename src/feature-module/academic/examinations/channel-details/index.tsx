...
const ChannelDetailTable = () => {
    ...
    const [newEnginner, setNewEnginner] = useState<Engineer>({
        channel_id: null, name: null, is_active:true, has_sub_category:false,
    })
    ...
    const fetchProviderData = async (page: number) => {
        ...
        const response = await fetch(`${webApi}/tickets/channels/?page=${page}`, options);
    ...
    useEffect(() => { if (!checkPermission("view", "Can view channel")) { Cookies.remove('authToken'); navigate('/login'); } else { fetchData(currentPage); } }, [currentPage, userPermissions, navigate])
    ...
    const updateData = async () => {
        ...
        const url = editingIndex !== null ? `${webApi}/tickets/channels/${newEnginner.channel_id}/` : `${webApi}/tickets/channels/`;
    ...
    useEffect(() => {
        if (currentRecord) {
            setNewEnginner({
                channel_id: currentRecord.channel_id,
                name: currentRecord.name,
                is_active: currentRecord.is_active,
                has_sub_category: currentRecord.has_sub_category
            });
            if (currentRecord.channel_id !== null) {
                setEditingIndex(Number(currentRecord.channel_id));
            }
        }
    }, [currentRecord]);
        ...
        },
        {
            ...
            render: (_text: string, record: Engineer) => (
              <div className="d-flex align-items-center">
                {checkPermission("change", "Can change channel") && (
                  <button
                ...
                <div className="content" style={{ backgroundImage: 'linear-gradient(160deg, #0093E9 0%, #80D0C7 100%)', minHeight: '99%', margin: '5px', marginTop: '15px', position: 'relative', maxWidth: '99%', overflowY: 'auto', overflowX: 'auto' }}>
                    ...
                    <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
                        <div className="my-auto mb-2">
                            <h3 className="page-title mb-1">Channel Details</h3>
                            <nav>
                        ...
                        <div className="d-flex my-xl-auto right-content align-items-center flex-wrap">
                            <div className="mb-2">
                            {checkPermission("add", "Can add channel") && (<button className="btn btn-primary" data-bs-toggle="modal" onClick={resetForm} data-bs-target="#add_Approaval" type="button" > <i className="ti ti-square-rounded-plus-filled me-2" /> Add New</button>)}
                            </div>
                        </div>
                    </div>
                    {/* /Page Header */}
                    ...
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Add Channel Details</h4>
                                <button
                                    type="button"
