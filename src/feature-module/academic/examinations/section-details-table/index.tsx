...
const SectionDetailTable = () => {
    ...
    const [currentRecord, setCurrentRecord] = useState<SectionDetail | null>(null);
    ...
    const [newSectionDetail, setNewSectionDetail] = useState<SectionDetail>({
      section: null, id: 0,
    })
    ...
    const fetchProviderData = async (page: number) => {
        ...
        const response = await fetch(`${webApi}/provider/section_connections/?page=${page}`, options);
    ...
    const fetchData = async (_page: number) => {
        try {
            ...
            setTotalPages(sectionData.total_pages)
            setSectionDetail(sectionData.results.data)
            setFrom(sectionData.from);
            setTo(sectionData.to);
            setTotalCount(sectionData.total_count)
    ...
    useEffect(() => { if (!checkPermission("view", "Can view section connection")) { Cookies.remove('authToken'); navigate('/login'); } else { fetchData(currentPage); } }, [currentPage, userPermissions, navigate])

    const updateData = async () => {
        const updatedSectionDetail = editingIndex !== null ? sectionDetail.map((sectionDetail, index) => index == editingIndex ? { ...newSectionDetail } : sectionDetail) : [...sectionDetail, { ...newSectionDetail }];
        const requestOptions = {
            ...
            body: JSON.stringify(newSectionDetail)
        };
        const url = editingIndex !== null ? `${webApi}/provider/section_connections/${newSectionDetail.id}/` : `${webApi}/provider/section_connections/`;
    ...
    const editApprovalStatus = (record: SectionDetail) => {
    ...
    useEffect(() => {
        if (currentRecord) {
            setNewSectionDetail({
                id: currentRecord.id, section: currentRecord.section
            })
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setNewSectionDetail({ ...newSectionDetail, [name]: value || null });
    };
    ...
    const columns = [
        ...
        },
        {
            title: "Action",
            dataIndex: "action",
            render: (_text: string, record: SectionDetail) => (
              <div className="d-flex align-items-center">
                {checkPermission("change", "Can change section connection") && (
                  <button
                ...
                <div className="content" style={{ backgroundImage: 'linear-gradient(160deg, #0093E9 0%, #80D0C7 100%)', minHeight: '99%', margin: '5px', marginTop: '15px', position: 'relative', maxWidth: '99%', overflowY: 'auto', overflowX: 'auto' }}>
                    ...
                    <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
                        <div className="my-auto mb-2">
                            <h3 className="page-title mb-1">Device Location</h3>
                            <nav>
                        ...
                        <div className="d-flex my-xl-auto right-content align-items-center flex-wrap">
                            <div className="mb-2">
                            {checkPermission("add", "Can add section connection") && (<button className="btn btn-primary" data-bs-toggle="modal" onClick={resetForm} data-bs-target="#add_Approaval" type="button" > <i className="ti ti-square-rounded-plus-filled me-2" /> Add New</button>)}
                            </div>
                        </div>
                    </div>
                    {/* /Page Header */}
        ...
        `}
                    ...
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Add Device Location</h4>
                                <button
                                    type="button"
                                ...
                                <div className="modal-body light-violet-bg">
                                    <div className="mb-3">
                                        ...
                                        <input
                                            ...
                                            name="section"
                                            placeholder="Name"
                                            value={newSectionDetail.section || ''}
                                        />
                                        {errorMessage.name && (
                ...
                <div className="modal fade" id="edit_Provider" tabIndex={-1} aria-hidden="true">
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Edit Device Location</h4>
                                <button
                                    type="button"
                                    className="btn-close custom-btn-close"
                                    data-bs-dismiss="modal"
