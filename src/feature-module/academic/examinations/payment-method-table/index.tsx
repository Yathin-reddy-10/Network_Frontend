...
type Engineer = {
    ...
    payment_method_name: number | null,
...
const PaymentMethodTable = () => {
    ...
    const [newEnginner, setNewEnginner] = useState<Engineer>({
        payment_method_id: null, payment_method_name: null, is_active:true,
    })
    ...
    const fetchProviderData = async (page: number) => {
        ...
        const response = await fetch(`${webApi}/provider/payment_method/?page=${page}`, options);
    ...
    useEffect(() => { if (!checkPermission("view", "Can view service type")) { Cookies.remove('authToken'); navigate('/login'); } else { fetchData(currentPage); } }, [currentPage, userPermissions, navigate])

    const updateData = async () => {
        const updatedProviderDetails = editingIndex !== null ? enginner.map((enginner, index) => index == editingIndex ? { ...newEnginner } : enginner) : [...enginner, { ...newEnginner }];
        ...
        const url = editingIndex !== null ? `${webApi}/provider/payment_method/${newEnginner.payment_method_id}/` : `${webApi}/provider/payment_method/`;
    ...
    useEffect(() => {
        if (currentRecord) {
            setNewEnginner({
                payment_method_id: currentRecord.payment_method_id,
                payment_method_name: currentRecord.payment_method_name,
                is_active: currentRecord.is_active,
            });
            if (currentRecord.payment_method_id !== null) {
                setEditingIndex(Number(currentRecord.payment_method_id));
            }
        }
    }, [currentRecord]);
        ...
        },
        {
            ...
            render: (_text: string, record: Engineer) => (
              <div className="d-flex align-items-center">
                {checkPermission("change", "Can change service type") && (
                  <button
                ...
                <div className="content" style={{ backgroundImage: 'linear-gradient(160deg, #0093E9 0%, #80D0C7 100%)', minHeight: '99%', margin: '5px', marginTop: '15px', position: 'relative', maxWidth: '99%', overflowY: 'auto', overflowX: 'auto' }}>
                    ...
                    <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
                        <div className="my-auto mb-2">
                            <h3 className="page-title mb-1">Payment Method Details</h3>
                            <nav>
                        ...
                        <div className="d-flex my-xl-auto right-content align-items-center flex-wrap">
                            <div className="mb-2">
                            {checkPermission("add", "Can add service type") && (<button className="btn btn-primary" data-bs-toggle="modal" onClick={resetForm} data-bs-target="#add_Approaval" type="button" > <i className="ti ti-square-rounded-plus-filled me-2" /> Add New</button>)}
                            </div>
                        </div>
                    </div>
                    {/* /Page Header */}
        ...
        `}
                    ...
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Add Payment Method Details</h4>
                                <button
                                    type="button"
                            ...
                            <form onSubmit={handleSubmit} onKeyPress={(e) => { if (e.key === 'Enter') { handleSubmit(e); } }}>
                                <div className="modal-body light-violet-bg">
                                    <div className="mb-3">
                                        ...
                                        <input
                                            type="text"
                                            className={`form-control ${errorMessage.payment_method_name ? 'is-invalid' : ''}`}
                                            onChange={handleInputChange}
                                            name="payment_method_name"
                                            placeholder=" Name"
                                            value={newEnginner.payment_method_name || ''}
                                        />
                                        {errorMessage.payment_method_name && (
                                            <div className="invalid-feedback">{errorMessage.payment_method_name[0]}</div>
                                        )}
                                    </div>
                                </div>
                ...
                <div className="modal fade" id="edit_Provider" tabIndex={-1} aria-hidden="true">
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Edit Payment Method Details</h4>
                                <button
                                    type="button"
                                    className="btn-close custom-btn-close"
                                    data-bs-dismiss="modal"
