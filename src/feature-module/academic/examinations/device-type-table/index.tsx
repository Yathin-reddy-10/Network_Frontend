...
const DeviceTypeTable = () => {
    ...
    const [currentRecord, setCurrentRecord] = useState<DeviceType | null>(null);
    ...
    const fetchProviderData = async (page: number) => {
        ...
        const response = await fetch(`${webApi}/provider/device_types/?page=${page}`, options);
        return await response.json();
    }

    const fetchData = async (_page: number) => {
        try {
            const [deviceTypeData] = await Promise.all([
                fetchProviderData(_page)
            ]);
            setTotalPages(deviceTypeData.total_pages)
            setDeviceType(deviceTypeData.results.data)
            setFrom(deviceTypeData.from);
            setTo(deviceTypeData.to);
            setTotalCount(deviceTypeData.total_count)
        }
        catch (error) {
    ...
    useEffect(() => { if (!checkPermission("view", "Can view device type")) { Cookies.remove('authToken'); navigate('/login'); } else { fetchData(currentPage); } }, [currentPage, userPermissions, navigate])

    const updateData = async () => {
        const updatedDeviceType = editingIndex !== null ? deviceType.map((deviceType, index) => index == editingIndex ? { ...newDeviceType } : deviceType) : [...deviceType, { ...newDeviceType }];
        ...
        const url = editingIndex !== null ? `${webApi}/provider/device_types/${newDeviceType.id}/` : `${webApi}/provider/device_types/`;
    ...
    const editApprovalStatus = (record: DeviceType) => {
    ...
    useEffect(() => {
        if (currentRecord) {
            setNewDeviceType({
                id: currentRecord.id, type: currentRecord.type
            })
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setNewDeviceType({ ...newDeviceType, [name]: value || null });
    };
    ...
    const columns = [
        ...
        },
        {
            title: "Action",
            dataIndex: "action",
            render: (_text: string, record: DeviceType) => (
              <div className="d-flex align-items-center">
                {checkPermission("change", "Can change device type") && (
                  <button
                ...
                <div className="content" style={{ backgroundImage: 'linear-gradient(160deg, #0093E9 0%, #80D0C7 100%)', minHeight: '99%', margin: '5px', marginTop: '15px', position: 'relative', maxWidth: '99%', overflowY: 'auto', overflowX: 'auto' }}>
                    ...
                    <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
                        ...
                        <div className="d-flex my-xl-auto right-content align-items-center flex-wrap">
                            <div className="mb-2">
                            {checkPermission("add", "Can add device type") && ( <button className="btn btn-primary" data-bs-toggle="modal" onClick={resetForm} data-bs-target="#add_Approaval" type="button" > <i className="ti ti-square-rounded-plus-filled me-2" /> Add New</button>)}
                            </div>
                        </div>
                    </div>
                    {/* /Page Header */}
        ...
        `}
                    ...
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Add Gateway Device Type</h4>
                                <button
                                    type="button"
                            ...
                            <form onSubmit={handleSubmit} onKeyPress={(e) => { if (e.key === 'Enter') { handleSubmit(e); } }}>
                                <div className="modal-body light-violet-bg">
                                    <div className="mb-3">
                                        ...
                                        />
                                        {errorMessage.type && (
                                            <div className="invalid-feedback">{errorMessage.type[0]}</div>
                                        )}
                                    </div>
                                </div>
                ...
                <div className="modal fade" id="edit_Provider" tabIndex={-1} aria-hidden="true">
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Edit Gateway Device Type</h4>
                                <button
                                    type="button"
                                    className="btn-close custom-btn-close"
                                    data-bs-dismiss="modal"
