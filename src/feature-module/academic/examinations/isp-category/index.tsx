...
const IspCategoryTable = () => {
    ...
    const fetchProviderData = async (page: number) => {
        ...
        const response = await fetch(`${webApi}/provider/ISP_category/?page=${page}`, options);
    ...
    const fetchData = async (_page: number) => {
        try {
            ...
            setAdmin(providerData.results.data)
    ...
    useEffect(() => { if (!checkPermission("view", "Can view isp category")) { Cookies.remove('authToken'); navigate('/login'); } else { fetchData(currentPage); } }, [currentPage, userPermissions, navigate])

    const updateData = async () => {
        const updatedProviderDetails = editingIndex !== null ? admin.map((admin, index) => index == editingIndex ? { ...newAdmin } : admin) : [...admin, { ...newAdmin }];
        ...
        const url = editingIndex !== null ? `${webApi}/provider/ISP_category/${newAdmin.id}/` : `${webApi}/provider/ISP_category/`;
    ...
    const editApprovalStatus = (record: Admin) => {
    ...
    useEffect(() => {
        if (currentRecord) {
            setNewAdmin({
                id: currentRecord.id, Category_name: currentRecord.Category_name
            })
        ...
        },
        {
            ...
            render: (_text: string, record: Admin) => (
              <div className="d-flex align-items-center">
                {checkPermission("change", "Can change isp category") && (
                  <button
                ...
                <div className="content" style={{ backgroundImage: 'linear-gradient(160deg, #0093E9 0%, #80D0C7 100%)', minHeight: '99%', margin: '5px', marginTop: '15px', position: 'relative', maxWidth: '99%', overflowY: 'auto', overflowX: 'auto' }}>
                    ...
                    <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
                        <div className="my-auto mb-2">
                            <h3 className="page-title mb-1">ISP Service Type</h3>
                            <nav>
                        ...
                        <div className="d-flex my-xl-auto right-content align-items-center flex-wrap">
                            <div className="mb-2">
                            {checkPermission("add", "Can add isp category") && (<button className="btn btn-primary" data-bs-toggle="modal" onClick={resetForm} data-bs-target="#add_Approaval" type="button" > <i className="ti ti-square-rounded-plus-filled me-2" /> Add New</button>)}
                            </div>
                        </div>
                    </div>
                    {/* /Page Header */}
        ...
        `}
                    ...
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Add ISP Service Type</h4>
                                <button
                                    type="button"
                            ...
                            <form onSubmit={handleSubmit} onKeyPress={(e) => { if (e.key === 'Enter') { handleSubmit(e); } }}>
                                <div className="modal-body light-violet-bg">
                                    <div className="mb-3">
                                        ...
                                        <input
                                            type="text"
                                            className={`form-control ${errorMessage.Category_name ? 'is-invalid' : ''}`}
                                            onChange={handleInputChange}
                                            name="Category_name"
                                            placeholder="ISP Service Type"
                                            value={newAdmin.Category_name || ''}
                                        />
                                        {errorMessage.Category_name && (
                                            <div className="invalid-feedback">{errorMessage.Category_name[0]}</div>
                                        )}
                                    </div>
                                </div>
                ...
                <div className="modal fade" id="edit_Provider" tabIndex={-1} aria-hidden="true">
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Edit ISP Service Type</h4>
                                <button
                                    type="button"
                                    className="btn-close custom-btn-close"
                                    data-bs-dismiss="modal"
...
export default IspCategoryTable