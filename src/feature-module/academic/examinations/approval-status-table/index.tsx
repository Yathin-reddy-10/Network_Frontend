...
const ApproavlStatusTable = () => {
    const routes = all_routes;
    const navigate = useNavigate();
    const [approvalStatus, setApprovalStatus] = useState<ApproavStatus[]>([]);
    ...
    const [currentRecord, setCurrentRecord] = useState<ApproavStatus | null>(null);
    ...
    const [newApproavlStatus, setNewApproavlStatus] = useState<ApproavStatus>({
      name: null, id: 0,
    })

    const resetForm = () => {
        setNewApproavlStatus({
            name: null, id: 0,
        })
    ...
    const fetchProviderData = async (page: number) => {
        ...
        const response = await fetch(`${webApi}/provider/approval_status/?page=${page}`, options);
    ...
    const fetchData = async (_page: number) => {
        try {
            ...
            setApprovalStatus(providerData.results.data)
    ...
    useEffect(() => { if (!checkPermission("view", "Can view ApprovalStatus")) { Cookies.remove('authToken'); navigate('/login'); } else { fetchData(currentPage); } }, [currentPage, userPermissions, navigate])

    const updateData = async () => {
        const updatedProviderDetails = editingIndex !== null ? approvalStatus.map((approvalStatus, index) => index == editingIndex ? { ...newApproavlStatus } : approvalStatus) : [...approvalStatus, { ...newApproavlStatus }];
        const requestOptions = {
            ...
            body: JSON.stringify(newApproavlStatus)
        };
        const url = editingIndex !== null ? `${webApi}/provider/approval_status/${newApproavlStatus.id}/` : `${webApi}/provider/approval_status/`;
        ...
        setApprovalStatus(updatedProviderDetails);
    ...
    const editApprovalStatus = (record: ApproavStatus) => {
    ...
    useEffect(() => {
        if (currentRecord) {
            setNewApproavlStatus({
                id: currentRecord.id, name: currentRecord.name
            })
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setNewApproavlStatus({ ...newApproavlStatus, [name]: value || null });
    };
    ...
    const columns = [
        ...
        },
        {
            title: "Action",
            dataIndex: "action",
            render: (_text: string, record: ApproavStatus) => (
              <div className="d-flex align-items-center">
                {checkPermission("change", "Can change ApprovalStatus") && (
                  <button
                ...
                <div className="content" style={{ backgroundImage: 'linear-gradient(160deg, #0093E9 0%, #80D0C7 100%)', minHeight: '99%', margin: '5px', marginTop: '15px', position: 'relative', maxWidth: '99%', overflowY: 'auto', overflowX: 'auto' }}>
                    ...
                    <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
                        <div className="my-auto mb-2">
                            <h3 className="page-title mb-1">Approval Status Details</h3>
                            <nav>
                        ...
                        <div className="d-flex my-xl-auto right-content align-items-center flex-wrap">
                            <div className="mb-2">
                            {checkPermission("add", "Can add ApprovalStatus") && (<button className="btn btn-primary" data-bs-toggle="modal" onClick={resetForm} data-bs-target="#add_Approaval" type="button" > <i className="ti ti-square-rounded-plus-filled me-2" /> Add New</button>)}
                            </div>
                        </div>
                    </div>
                    {/* /Page Header */}
        ...
        `}
                    ...
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Add New Approaval Status</h4>
                                <button
                                    type="button"
                ...
                <div className="modal fade" id="edit_Provider" tabIndex={-1} aria-hidden="true">
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Edit Approaval Status</h4>
                                <button
                                    type="button"
                                    className="btn-close custom-btn-close"
                                    data-bs-dismiss="modal"
...
export default ApproavlStatusTable