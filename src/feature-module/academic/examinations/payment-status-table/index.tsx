...
const PaymentStatusTable = () => {
    ...
    const [currentRecord, setCurrentRecord] = useState<PaymentStatus | null>(null);
    ...
    const [newPayamntStatus, setNewPaymentStatus] = useState<PaymentStatus>({
      name: null, id: 0,
    })
    ...
    const fetchProviderData = async (page: number) => {
        ...
        const response = await fetch(`${webApi}/provider/payment_status/?page=${page}`, options);
        return await response.json();
    }

    const fetchData = async (_page: number) => {
        try {
            const [paymentStatusData] = await Promise.all([
                fetchProviderData(_page)
            ]);
            setTotalPages(paymentStatusData.total_pages)
            setPaymentStatus(paymentStatusData.results.data)
            setFrom(paymentStatusData.from);
            setTo(paymentStatusData.to);
            setTotalCount(paymentStatusData.total_count)
        }
        catch (error) {
    ...
    useEffect(() => { if (!checkPermission("view", "Can view PaymentStatus")) { Cookies.remove('authToken'); navigate('/login'); } else { fetchData(currentPage); } }, [currentPage, userPermissions, navigate])

    const updateData = async () => {
        const updatedpaymentStatusDetails = editingIndex !== null ? paymentStatus.map((paymentStatus, index) => index == editingIndex ? { ...newPayamntStatus } : paymentStatus) : [...paymentStatus, { ...newPayamntStatus }];
        const requestOptions = {
            ...
            body: JSON.stringify(newPayamntStatus)
        };
        const url = editingIndex !== null ? `${webApi}/provider/payment_status/${newPayamntStatus.id}/` : `${webApi}/provider/payment_status/`;
        ...
        setPaymentStatus(updatedpaymentStatusDetails);
    ...
    const editApprovalStatus = (record: PaymentStatus) => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setNewPaymentStatus({ ...newPayamntStatus, [name]: value || null });
    };
    ...
    const columns = [
        ...
        },
        {
            title: "Action",
            dataIndex: "action",
            render: (_text: string, record: PaymentStatus) => (
              <div className="d-flex align-items-center">
                {checkPermission("change","Can change PaymentStatus") && (
                  <button
                ...
                <div className="content" style={{ backgroundImage: 'linear-gradient(160deg, #0093E9 0%, #80D0C7 100%)', minHeight: '99%', margin: '5px', marginTop: '15px', position: 'relative', maxWidth: '99%', overflowY: 'auto', overflowX: 'auto' }}>
                    ...
                    <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
                        <div className="my-auto mb-2">
                            <h3 className="page-title mb-1">Payment Status Details</h3>
                            <nav>
                        ...
                        <div className="d-flex my-xl-auto right-content align-items-center flex-wrap">
                            <div className="mb-2">
                                {checkPermission("add", "Can add PaymentStatus")&&(<button className="btn btn-primary" data-bs-toggle="modal" onClick={resetForm} data-bs-target="#add_Approaval" type="button" > <i className="ti ti-square-rounded-plus-filled me-2" /> Add New</button>)}
                            </div>
                        </div>
                    </div>
                    {/* /Page Header */}
                    ...
                    <div className="modal-dialog custom-modal-size">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h4 className="modal-title">Add New Payment Status</h4>
                                <button
                                    type="button"
